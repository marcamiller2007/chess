...
void Ruleset::ForPawn() {
	...
	int direction = (is_black_ == current_square_->PlayerIsBlack()) ? (-1) : 1;

	// can move 1 or 2 squares forward
	for (int i = 1; i <= max; i++) {
		Board::Square *square = current_square_->FindNeighbor(
			{initial_position.x + (i * direction), initial_position.y}
		);
	...
	cap = current_square_->FindNeighbor(
		{initial_position.x + direction, initial_position.y + 1}
	);
	if ((cap != nullptr) && cap->IsPiece() && (cap->GetPiece()->kIsBlack != is_black_)) {
